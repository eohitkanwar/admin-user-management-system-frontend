...
export const getUserActivities = async (page = 1, limit = 10, search = '') => {
  try {
    ...
    const { data } = await api.get(`/auth/activities?page=${page}&limit=${limit}&search=${search}`);
...
export const createActivityLog = async (activityData) => {
  try {
    ...
    const { data } = await api.post('/auth/activities', activityData);
...
export const getActivitiesByAdmin = async (adminId, page = 1, limit = 10) => {
  try {
    ...
    const { data } = await api.get(`/auth/activities/admin/${adminId}?page=${page}&limit=${limit}`);
